...
func startChrome(exe, profile, extDir, extID string, background bool) (*exec.Cmd, error) {
	cmd := exec.Command(exe, shoppingChromeArgs(profile, extDir, extID, background)...)
	unlock := func() {}
	if background {
